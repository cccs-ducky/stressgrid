defmodule Statsd do
  ...
  You can read more about all the options in the `Statsd` module documentation.
  ...
  @doc false
  @spec get_formatter(pid()) :: module()
  def get_formatter(reporter) do
    GenServer.call(reporter, :get_formatter)
  end
...
#  end
  ...
  @doc """
  Publishes a gauge metric directly to StatsD.

  ## Parameters
  - `name`: The metric name (string or atom list)
  - `value`: The gauge value (number)
  - `tags`: Optional keyword list of tags (default: [])

  ## Examples
      Statsd.gauge("memory.usage", 1024)
      Statsd.gauge([:vm, :memory, :total], 2048, [node: "web01"])
  """
  @spec gauge(String.t() | [atom()], number(), keyword()) :: :ok | :error
  def gauge(name, value, tags \\ []) do
    publish_metric(:last_value, name, value, tags)
  end

  @doc """
  Publishes a counter metric directly to StatsD.

  ## Parameters
  - `name`: The metric name (string or atom list)
  - `increment`: The counter increment value (default: 1)
  - `tags`: Optional keyword list of tags (default: [])

  ## Examples
      Statsd.counter("http.requests")
      Statsd.counter([:http, :requests], 5, [status: 200])
  """
  @spec counter(String.t() | [atom()], number(), keyword()) :: :ok | :error
  def counter(name, increment \\ 1, tags \\ []) do
    publish_metric(:counter, name, increment, tags)
  end

  @doc """
  Publishes a histogram metric directly to StatsD.

  ## Parameters
  - `name`: The metric name (string or atom list)
  - `value`: The histogram value (number)
  - `tags`: Optional keyword list of tags (default: [])

  ## Examples
      Statsd.histogram("response.time", 120)
      Statsd.histogram([:db, :query, :duration], 45.2, [table: "users"])
  """
  @spec histogram(String.t() | [atom()], number(), keyword()) :: :ok | :error
  def histogram(name, value, tags \\ []) do
    publish_metric(:summary, name, value, tags)
  end
    ...
    case Process.whereis(__MODULE__) do
      ...
      pid ->
        ...
        case get_udp_worker(pool_id) do
          ...
          :error ->
            Logger.error("Failed to publish metric: no UDP worker available")
            :error
        end
    end
  end

  defp normalize_metric_name(name) when is_binary(name) do
    String.split(name, ".") |> Enum.map(&String.to_atom/1)
  end

  defp normalize_metric_name(name) when is_list(name) do
    name
  end

  defp create_metric(:counter, name) do
  ...
  defp create_metric(:last_value, name) do
  ...
  defp create_metric(:summary, name) do
