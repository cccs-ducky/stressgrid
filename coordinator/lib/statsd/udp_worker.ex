defmodule Statsd.UDPWorker do
  @moduledoc false

  use GenServer

  alias Statsd.{UDP, Packet, CounterOk, CounterError}
  ...
  defp maybe_send_udp_datagrams(state, datagrams \\ nil) do
    ...
    if datagrams_to_send != [] do
      case Statsd.get_udp(state.pool_id) do
        {:ok, udp} ->
          case UDP.send(udp, datagrams_to_send) do
            ...
            {:error, reason} ->
              ...
              Statsd.udp_error(state.reporter, udp, reason)
          end
